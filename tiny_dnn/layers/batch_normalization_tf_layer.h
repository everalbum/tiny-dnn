 ...
 **/
class batch_normalization_tf_layer : public layer {
 public:
  typedef layer Base;
   ...
   **/
  batch_normalization_tf_layer(const layer &prev_layer,
   ...
   * @param phase           [in] specify the current context (train/test)
   **/
  batch_normalization_tf_layer(size_t in_spatial_size,
                            size_t in_channels,
                            float_t epsilon  = 1e-5,
                            float_t momentum = 0.999,
                            net_phase phase  = net_phase::train)
  ...
  virtual ~batch_normalization_tf_layer() {}
  ...
  void forward_propagation(const std::vector<tensor_t *> &in_data,
    ...
    vec_t &beta = (phase_ == net_phase::train) ? beta_current_ : beta_;
  ...
  void set_beta(const vec_t &beta) {
    beta_ = beta;
  }
 ...
 private:
  ...
  void init() {
    ...
    beta_current_.resize(in_channels_);
    beta_.resize(in_channels_);
